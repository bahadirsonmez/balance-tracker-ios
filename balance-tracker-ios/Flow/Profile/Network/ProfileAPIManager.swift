...
class ProfileAPIManager {
    ...
    func callWH<T>(router: ProfileRouter, handler: @escaping (Swift.Result<T, APIError>) -> Void) where T: Codable {
    ...
    func call<T>(router: ProfileRouter, handler: @escaping (Swift.Result<T, APIError>) -> Void) where T: Codable {
    ...
    func call(router: ProfileRouter, handler: @escaping([ProfileItem]?, Error?) -> Void) {
        self.sessionManager.request(router).validate(statusCode: 200..<300).responseJSON { (response) in
            ...
            case .success:
                if let jsonData = response.data {
                    ...
                    do {
                        let workoutCards = try jsonDecoder.decode([ProfileItem].self, from: jsonData)
    ...
    func callResponse(router: ProfileRouter, handler: @escaping(String?, SkyErrorModel?) -> Void){
...
//        //        KeychainAccount.sharedAccount.setRefreshToken(refreshToken)
//    }
